...
void MyRTPSession::OnPollThreadStep()
{
	...
	std::cout << "BeginDataAccess " << BeginDataAccess()<< std::endl;	
		
	// check incoming packets
	if (GotoFirstSourceWithData())
	{
		//std::cout << "GotoFirstSourceWithData" << std::endl;	
		do
		{
			RTPPacket *pack;
	...
	//std::cout << "OnPollThreadStep enddata" << std::endl;	
	EndDataAccess();
}
...
void MyRTPSession::ProcessRTPPacket(const RTPSourceData &srcdat,const RTPPacket &rtppack)
{
	...
	//std::cout << "Got packet " << rtppack.GetExtendedSequenceNumber()<< std::endl;	
	//std::cout << " from SSRC " << srcdat.GetSSRC()<< std::endl;	
	//std::cout << "data = " << rtppack.GetPacketData()<< std::endl;	
	std::cout << "data = [" << count_pak << "]" << rtppack.GetPayloadData() << std::endl;
	count_pak++;
}
